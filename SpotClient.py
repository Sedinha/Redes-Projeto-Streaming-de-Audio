...
def carregarMSC(socketCliente, nome_msc: str):
	...
	if resp != "track_data_start":
		return
	t1 = threading.Thread(target=baixarMusica, args=(nome_msc,socketCliente), daemon=True)
	t1.start()
    ...
    if comando == "pause" or comando == "6" :
        ...
        if musica:
            ...
            if musica not in listaCacheLocal:
                print(f"Musica '{musica}' não encontrada. Please send 'lista' para ver a lista de musicas")
                continue
            ...
            time.sleep(3.0)
            t=threading.Thread(target=tocarMusica, args=(musica,), daemon=True)
            t.start()
        else:
